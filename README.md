...
- **Read offsets from the `claims[]` array.** In `SearchJobResponse`, `statement_offset`/`statement_length` are populated only on canonical `claims[]`. The `unified_results[]` and `evidence_documents[].extracted_claims[]` projections carry `extraction_fidelity`/`match_method`/`source_assertion_opinion` but return `statement_offset: null`. **Join spans by `claims[]` for offset-based matching.** Offsets are **Python character offsets** — validate with string slicing, not bytes. **Post-CC-10a:** `evidence_documents[].extracted_claims` carry `statement_offset/length` **by default** (offsets are not gated), and `source_text` is returned **opt-in** via `include_source_text=true` — making each evidence document **self-contained for annotation**; prefer that to the top-level `claims[]` cross-join. Validate `source_text[offset:offset+len] == verbatim_span` (the extractor's verbatim `text_snippet`), not normalized claim text.
...
- **⚠ Full `source_text` is NOT returned by the API (blocks T2 on fetched docs — needs CC-10).** `EvidenceDocument` has **no `source_text` field** — only bounded `passages` (≤3 × ≤1200 chars) + optional `raw_context` (≤4000, default off). But extracted-claim offsets index into a full source_text (observed `statement_offset=8516`) that is never returned. So:
  - **Mode A `/search`** on a *fresh* query **does** return `evidence_document_v1` (the "legacy_claim_fragment" the first probe saw was a **cache effect** — `/search` mixes in cached/legacy KB claims; `legacy_claim_fragment_count:0` on cache-miss). It is **not** the blocker.
  - The real blocker is **no full `source_text` for any fetched document** (Mode A *and* `/extract`-by-URL). Span annotation (T2) on fetched docs is **blocked until CC-10a** (response-only: opt-in `source_text` + always-on per-claim offsets on evidence docs → self-contained annotation). **CC-10b** (`full_document_extraction=true`, persisted server-side, idempotency-honest) is a *separate* later commit needed only for *long* docs where bounded extraction truncates.
  - **Only `raw_text` Mode B gives a usable source_text today** — because the client supplied the text. **Do not trust `capture._reconstruct_source_text()` (stitching ≤3 passages) for offset scoring** — it's sparse/gappy and offsets won't align. Mark fetched-doc fixtures as `not span-annotatable` until CC-10.
  - **Cache awareness:** for clean extraction eval, force cache-miss queries or filter on `evidence_diagnostics` (`legacy_claim_fragment_count`, `evidence_document_missing_reason_counts`); don't score legacy fragments as extraction failures.
...
- **⚠ source_text availability (see §3):** T2 needs the full `source_text`. Today that's only reliably present for **`raw_text` Mode B** captures. Fetched docs (Mode A / `/extract`-by-URL) don't return full `source_text` (passage-only) — they're **not span-annotatable until CC-10** lands `include_source_text=true`. The capture client's passage-reconstruction is **not** offset-faithful; don't score against it.
