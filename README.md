...
- **✓ Full `source_text` + offsets now captured for all fetched docs (CC-10a/10b live, verified 2026-06-17).** Not just `raw_text` — Mode A and `/extract`-by-URL too. How it works:
  - **Mode A `/search` is async:** the **poll GET must pass `?include_evidence_documents=true&include_source_text=true`** — without them the response has `evidence_documents: null` (this was the original capture bug; the "legacy_claim_fragment" was a **cache effect**, not a mode). The client now does this.
  - **CC-10a:** evidence docs carry full `source_text`; per-claim `statement_offset/length` + `verbatim_span` live on `evidence_documents[].extracted_claims`. The client reads `source_text` directly (no lossy passage reconstruction) and attaches `verbatim_span` to each span.
  - **CC-10b:** the client sends `full_document_extraction=true` per request (config `capture_full_document_extraction`) so captures aren't truncated — no shared-server flag needed.
  - **Validate against `verbatim_span`, not `claim.statement`** (the latter may be normalized/repaired): `source_text[offset:offset+length] == verbatim_span`. Verified live **25/25**.
...
- **✓ source_text availability (see §3):** CC-10a/10b are live; the capture client captures full `source_text` + per-span `verbatim_span` for **all** fetched docs (Mode A and `/extract`-by-URL), verified `source_text[offset:offset+length] == verbatim_span` (25/25). Score offset/IoU against `verbatim_span`, not the (possibly normalized) `claim.statement`.
